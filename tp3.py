...
for i in range(0, nb_datasets):
    ...
    # We only keep the 8 first componenet given that after that, each component account for less than 5% of variance
...
# Define numbers of nearest neighbors
k_nn = 10

# Re-arrange known ratings in a m users X n items matrix
...
# Re-arrange test and base known ratings in a m users X n items matrix
for i in range(0, nb_datasets):
...
for i in tqdm(range(0, nb_datasets), desc='Data Sets'):
    ...
    if not path.Path(fname_dist_table).exists():
        ...
        # Save k nearest neihgbors table for re-use
    ...
    pred_rmse = list()
    # for each of the nearest neighbors, append test user and get cosine similarity
    for index, row in nearest_neighbors[i].iterrows():
        m = pd.concat([merged_data.iloc[row, :].T, merged_data.iloc[index - 1, :]], axis=1)
        s = pd.DataFrame(cosine_similarity(m.T), index=[*row, index], columns=[*row, index])
        # get weights from similiraty table, only take last element as it is test user weights
        w = s.iloc[:-1:, -1:].T

        # Set the minimum number of ratings
        gamma = 5
        # Get the number of common rating between test user and current neighbor
        for k in range(0, w.shape[1]):
            I_uv = ((m.iloc[:, k] != 0) & merged_data.iloc[index - 1, :] != 0).sum()
            w.iloc[0, k] = w.iloc[0, k] * min(gamma, I_uv)/gamma

        # Get predictions multiplied by weight
        weighted_votes = pd.DataFrame()

        for i in range(1, 6):
            _ratings = (m.iloc[:, :-1:] == (i/5)).multiply(w.to_numpy()).sum(axis=1)
            weighted_votes = pd.concat([weighted_votes, _ratings], axis=1)
        ...
        original rating predictions[merged_data.iloc[index - 1, :] != 0]
        '''
        predictions = weighted_votes.idxmax(axis=1)
        rmse = np.sqrt(mean_squared_error(predictions[merged_data.iloc[index - 1, :] != 0],
                                          m[merged_data.iloc[index - 1, :] != 0].iloc[:, k_nn] * 5))
...
sns.barplot(data=rmse_per_dataset, color='g')
sns.barplot(data=rmse_rdn_per_cluster, color='r')
plt.ylim([0, 2.4])
plt.title(f'Random vector RMSE vs. Collaborative filtering prediction RMSE, {k_nn} NNs')
plt.ylabel('RMSE')
plt.ylabel(f'Colaborative filtering TMSE {k_nn} NNs')
plt.show()
plt.savefig(f'images/method4_rmse_bar_{k_nn}knn.png')
